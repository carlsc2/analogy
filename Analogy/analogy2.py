...
def find_group_analogy(src_concepts, src_domain, target_domain):
    '''Finds the best analogy that does not conflict with the src_concepts
    src_concepts is a list of concepts in the source domain    
    '''
    ...
    for src_concept in src_concepts:
        ...
        for r, d, v in net1:
            ...
            if v: #if outgoing rtype
                if k in src_vec_dict:
                    src_vec_dict[k] += svec - src_domain.rtype_vectors[r]
                else:
                    src_vec_dict[k] = svec - src_domain.rtype_vectors[r]
            else: #if incoming rtype
                if k in src_vec_dict:
                    src_vec_dict[k] += svec - permute_rtype_vector(src_domain.rtype_vectors[r])
                else:
                    src_vec_dict[k] = svec - permute_rtype_vector(src_domain.rtype_vectors[r])
    ...
    def get_confidence(r1,r2):
        ...
        return 1 - (ratio1 - ratio2)**2

    for target_concept in target_domain.nodes:
